...
eventListeners() {
    ...
    data.addEventListener('click',  (e) => {
        if(event.target === incomePlus || event.target === expensesPlus) {
    ...
    expensesPlus.addEventListener('click', () => {
        if (expensesItem.length === 2 ){
            for (let i = 0; i < expensesItem[1].querySelectorAll('input').length; i++){
                expensesItem[1].querySelectorAll('input')[i].value = ''; 
            }
        }
        if (expensesItem.length === 3){
        for (let i = 0; i < expensesItem[1].querySelectorAll('input').length; i++){
            expensesItem[2].querySelectorAll('input')[i].value = ''; 
        }
        }              
